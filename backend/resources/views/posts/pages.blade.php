						...
						@foreach ($formFields as $name => $field)
						    <div class="mb-3 fv-row">
						        ...
						        @if ($field['type'] === 'select' && isset($field['options']))
						            <select name="{{ $name }}" id="{{ $name }}" class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}" 
						        ...
						        @elseif ($field['type'] === 'textarea')
						            <textarea id="{{ $name }}" name="{{ $name }}" class="form-control" 
						                      placeholder="{{ $field['placeholder'] }}">{{ old($name, $page->$name ?? '') }}</textarea>
						        @elseif ($field['type'] === 'ckeditor')
						            {{-- CKEditor Implementation --}}
						            <div id="{{ $name }}_toolbar"></div>
						            <div id="{{ $name }}_editor">{{ old($name) }}</div>
						            <textarea name="{{ $name }}" id="{{ $name }}" class="d-none">{{ old($name) }}</textarea>
						        @else
						    ...
						    <div class="mb-3 fv-row">
						        ...
						        @if ($field['type'] === 'select' && isset($field['options']))
						            <select name="{{ $name }}" id="{{ $name }}" 
						                ...
						                <option value="" disabled selected>{{ $field['placeholder'] ?? 'Select an option' }}</option>
						                @foreach ($field['options'] as $id => $label)
						                    <option value="{{ $id }}" {{ old($name, $page->$name ?? '') == $id ? 'selected' : '' }}>
						                        {{ $label }}
						                    </option>
						                @endforeach
						            </select>
						        ...
						        @elseif ($field['type'] === 'ckeditor')
						            {{-- CKEditor Implementation --}}
						            <div id="edit_{{ $name }}_toolbar"></div> <!-- Toolbar for CKEditor -->
						            <div id="edit_{{ $name }}_editor">{!! old($name, $page->$name ?? '') !!}</div> <!-- CKEditor content -->
						            <textarea name="{{ $name }}" id="edit_{{ $name }}" class="d-none">{{ old($name, $page->$name ?? '') }}</textarea> <!-- Hidden field for form submission -->
...
@section('compute')
						        ...
						        if (input.length) {
						            switch (input.attr("type")) {
						                case "checkbox":
						                    ...
						                    break;
						                case "file":
						                    input.val(""); // Clear file input for security reasons
						                    break;
						                default:
	...
	document.addEventListener("DOMContentLoaded", function () {
	    function initializeCKEditor(fieldName) {
	        let editorElement = document.getElementById(`${fieldName}_editor`);

	        if (editorElement) {
	            // Destroy existing CKEditor instance if it already exists
	            if (editorElement.ckeditorInstance) {
	                editorElement.ckeditorInstance.destroy()
	                    .then(() => createCKEditor(editorElement, fieldName))
	                    .catch(error => console.error("Error destroying CKEditor:", error));
	            } else {
	                createCKEditor(editorElement, fieldName);
	            }
	        }
	    }

	    function createCKEditor(editorElement, fieldName) {
	        DecoupledEditor
	            .create(editorElement)
	            .then(editor => {
	                editorElement.ckeditorInstance = editor; // Store reference

	                const toolbarContainer = document.getElementById(`${fieldName}_toolbar`);
	                toolbarContainer.appendChild(editor.ui.view.toolbar.element);

	                // Sync CKEditor content to hidden textarea before submitting
	                editor.model.document.on('change:data', () => {
	                    document.getElementById(fieldName).value = editor.getData();
	                });

	                // Set existing content for update modal or clear for create modal
	                const existingValue = document.getElementById(fieldName).value;
	                editor.setData(existingValue ? existingValue : ""); // Load data or clear
	            })
	            .catch(error => console.error("Error initializing CKEditor:", error));
	    }

	    // Initialize CKEditor when the create modal is opened (clear content)
	    $('#kt_modal_create').on('shown.bs.modal', function () {
	        document.getElementById("content").value = ""; // Ensure textarea is empty
	        initializeCKEditor("content");
	    });

	    // Initialize CKEditor when the update modal is opened (set existing content)
	    ...
	    $('#kt_modal_create').on('hidden.bs.modal', function () {
	        ...
	        if (editorElement && ckEditors["content"]) {
	            ckEditors["content"].destroy().catch(error => console.error("Error destroying CKEditor:", error));
	            delete ckEditors["content"]; // Clean up the editor instance
	        }
	    });

	    $('#kt_modal_update').on('hidden.bs.modal', function () {
	        const editorElement = document.getElementById("content_editor");
	        CKEDITOR.instances.editorElement.destroy();

	        if (editorElement && editorElement.ckeditorInstance) {
	            editorElement.ckeditorInstance.destroy().catch(error => console.error("Error destroying CKEditor:", error));
	        }
	    });
